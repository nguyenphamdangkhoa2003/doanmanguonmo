<div>
    ...
    <div class="grid grid-cols-4 gap-5">
        ...
        <div class="lg:col-span-1 col-span-4">
            <div class="lg:block grid grid-cols-2 gap-5">
                <div class="bg-white shadow lg:my-5 p-5">
                    <div class="flex justify-between w-full items-center">
                        <h3 class="text-2xl font-extrabold">Customer</h3>
                        <a href="{{route('list-user')}}"
                            class="link hover:text-primary text-gray-300 no-underline text-sm">View All</a>
                    </div>
                    <hr>
                    <div class="py-2">
                        @foreach ($users as $u)
                            @if (isset($u->avatar))
                                <div class="py-2 flex justify-between w-full items-center">
                                    <x-mary-avatar :image="$u->avatar->url" :title="$u->name"
                                        :subtitle="$u->created_at->format('M j, Y')" class="!w-10" />
                ...
                <div class="bg-white shadow p-5">
                    ...
                    <hr>
                    <x-mary-chart class="lg:w-52 w-auto" wire:model="rooms_chart" />
                </div>
            </div>
        </div>
    </div>
